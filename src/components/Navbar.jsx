  ...
  return (
    <nav
      ...
      className={`animate-nav-glow fixed top-4 left-1/2 -translate-x-1/2 z-50
        w-[92%] max-w-4xl rounded-full border border-white/10 px-6 py-3 backdrop-blur-xl
        transition-colors duration-500
        ${scrolled ? 'bg-[rgba(10,10,20,0.92)]' : 'bg-[rgba(10,10,20,0.65)]'}`}
    >
      <div className="relative w-full">
        <div className="grid grid-cols-[1fr_auto_1fr] items-center gap-3">
          {/* Left: Name */}
          <a href="#home" className="justify-self-start font-bold text-white text-lg tracking-wide hover:text-neon-pink transition-colors duration-300 whitespace-nowrap min-w-0">
            Samiha Shahin
          </a>
          ...
          <div className="hidden md:flex items-center justify-center gap-1 justify-self-center">
            {links.map((l) => (
              <a
                key={l.href}
                href={l.href}
                className="relative px-4 py-1.5 text-sm font-medium text-gray-300 rounded-full
          ...
          <div className="flex items-center justify-end gap-3 justify-self-end w-full min-w-0">
            <a
              href="#contact"
              id="hire-me-btn"
